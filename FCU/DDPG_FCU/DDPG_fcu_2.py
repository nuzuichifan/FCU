...
var_0 = 3   # the controller of exploration which will decay during training process
var_1 = 3   #
for i_episode in range(20):
    for i in range(4392):
        ...
        if s > 0:
            ...
            a = ddpg.choose_action(s_normalized)
            a = (a.numpy()).squeeze()
            # print(a)
            a[0] = np.clip(np.random.normal(a[0], var_0), -2, 2)
            a[1] = np.clip(np.random.normal(a[1], var_1), -2, 2)
...
#水流量：(210-580)step5  75；风量：（210-1020）step10  82
           ...
           # r = -math.fabs(s - Q)-(Pfan+Ppump)/3-Punish_coefficient
            ...
            ddpg.store_transition(s_normalized.squeeze(), a, r.squeeze() / 1500,
                                  s__normalized.squeeze())  # store the transition to memory
            # print(s_normalized, a, r / 5000, s__normalized)
            if ddpg.pointer > MEMORY_CAPACITY:

                if var_0 > 0.1:
                    var_0 *= 0.9995  # decay the exploration controller factor
                    var_1 *= 0.9995
                else:
                    var_0 = 0.1  # decay the exploration controller factor
                    var_1 = 0.1
